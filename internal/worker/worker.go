...
type Worker struct{Store *store.Store;Crawler *crawl.Crawler;Prober *probe.Prober;MaxPages int;MaxCandidatesPerPage int}
func(w *Worker)Run(ctx context.Context,job string)error{pages:=0;for pages<w.MaxPages{if ctx.Err()!=nil{return ctx.Err()};id,raw,domain,_,e:=w.Store.ClaimCandidate(ctx,job);if e!=nil{if errors.Is(e,context.Canceled)||errors.Is(e,context.DeadlineExceeded){return e};return w.Store.FinishJobIfEmpty(ctx,job)}
 pr,e:=w.Prober.Probe(ctx,domain);if e!=nil{_ = w.Store.SetCandidateStatus(ctx,id,"completed",pr.ErrorMessage);_ = w.Store.UpsertSignal(ctx,domain,"probe","active","false",0.9,nil,raw);continue}
 _ = w.Store.UpsertSignal(ctx,domain,"probe","active","true",1,nil,raw)
 p,e:=w.Crawler.Fetch(ctx,raw);if e!=nil{_ = w.Store.SetCandidateStatus(ctx,id,"failed_retryable",e.Error());continue};pages++
 tech:=detect.Technologies(p.Body,p.URL);wp:=false;wc:=false
 for _,t:=range tech{_ = w.Store.UpsertSignal(ctx,domain,"technology",t.Name,t.Version,t.Confidence,t.Evidence,t.URL);if t.Name=="WordPress"{wp=true};if t.Name=="WooCommerce"{wc=true}}
