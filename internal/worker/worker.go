...
func(w *Worker)Run(ctx context.Context,job string)error{
 ...
 for pages<w.MaxPages&&processed<w.MaxURLs{
  ...
  if !wp&&!wc{continue}
  for i,link:=range p.Links{if i>=w.MaxCandidatesPerPage{break};u,e:=urlnorm.URL(link);if e!=nil{continue};if policy.ValidateURL(link)!=nil{continue};if !strings.EqualFold(u.Hostname(),domain){continue};d,_:=urlnorm.Domain(link);_ = w.Store.UpsertCandidate(ctx,uuid.NewString(),job,link,u.String(),d,u.Hostname(),"deep_crawl",raw,50,0.8)}
 }
