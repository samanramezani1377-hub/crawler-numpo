...
func TestCreateJobReturnsJSONDecoderDetails(t *testing.T) {
 ...
 rr := httptest.NewRecorder()
 s.ServeHTTP(rr, req)
 if rr.Code != http.StatusBadRequest { t.Fatalf("status=%d, want 400", rr.Code) }
 var got map[string]any
 if err := json.Unmarshal(rr.Body.Bytes(), &got); err != nil { t.Fatalf("response is not JSON: %v", err) }
 e, ok := got["error"].(map[string]any); if !ok { t.Fatalf("missing error object: %#v", got) }
 if e["code"] != "invalid_json" { t.Fatalf("code=%v, want invalid_json", e["code"]) }
 if !strings.Contains(e["message"].(string), "trailing") { t.Fatalf("message=%v, want decoder detail", e["message"]) }
}
