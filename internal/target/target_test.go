...
func TestMatchTechnologiesAndPhone(t *testing.T) {
	...
	if !c.Match("https://shop.example.com", "<html>", []string{"WordPress", "WooCommerce"}, true) { t.Fatal("expected match") }
	if c.Match("https://shop.example.com", "<html>", []string{"WordPress"}, true) { t.Fatal("missing technology should not match") }
	if c.Match("https://shop.example.com", "<html>", []string{"WordPress", "WooCommerce"}, false) { t.Fatal("phone target should not match") }
}
