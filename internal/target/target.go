...
func FromMap(v map[string]any) Config {
	...
	if xs, ok := v["technologies"].([]any); ok {
		for _, x := range xs {
			if s, ok := x.(string); ok && strings.TrimSpace(s) != "" {
				c.Technologies = append(c.Technologies, strings.ToLower(strings.TrimSpace(s)))
			}
		}
	}
	...
	if xs, ok := v["country"].([]any); ok {
		for _, x := range xs {
			if s, ok := x.(string); ok && strings.TrimSpace(s) != "" {
				c.Countries = append(c.Countries, strings.ToLower(strings.TrimSpace(s)))
			}
		}
	}
	...
	if b, ok := v["has_public_phone"].(bool); ok {
		c.HasPublicPhone = &b
	}
	return c
}
...
func (c Config) Match(pageURL, body string, technologies []string, hasPublicPhone bool) bool {
	for _, wanted := range c.Technologies {
		...
		for _, got := range technologies {
			if strings.EqualFold(strings.TrimSpace(got), wanted) {
				found = true
				break
			}
		}
	...
	if c.HasPublicPhone != nil && *c.HasPublicPhone != hasPublicPhone {
		return false
	}
	if len(c.Countries) > 0 {
		...
		found := false
		for _, wanted := range c.Countries {
			if strings.EqualFold(country, wanted) {
				found = true
				break
			}
		}
...
func inferCountry(rawURL, body string) string {
	if u, err := url.Parse(rawURL); err == nil {
		...
		if len(parts) >= 2 {
			...
			if len(parts) >= 3 && tld == "uk" && parts[len(parts)-2] == "co" {
				return "gb"
			}
		}
	}
