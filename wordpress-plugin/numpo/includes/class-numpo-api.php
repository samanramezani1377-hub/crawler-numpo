...
class Numpo_API {
 ...
 public static function routes(){
  ...
  register_rest_route('numpo/v1','/jobs/(?P<id>[A-Za-z0-9-]+)/export/(?P<resource>candidates|urls|domains|pages|technologies|contacts|business|social|classifications|probes|errors)',['methods'=>'GET','permission_callback'=>[__CLASS__,'permission'],'callback'=>[__CLASS__,'export_resource']]);
 ...
 public static function resume(WP_REST_Request $r){$job=Numpo_DB::get_job($r['id']);if(!$job)return new WP_Error('not_found','Job not found.',['status'=>404]);if($job['status']!=='paused')return new WP_Error('invalid_state','Only paused jobs can be resumed.',['status'=>409]);Numpo_DB::set_job_status($r['id'],'running');Numpo_Worker::schedule($r['id']);return self::ok(['job_id'=>$r['id'],'status'=>'running']);}
 public static function export_resource(WP_REST_Request $r){$job=Numpo_DB::get_job($r['id']);if(!$job)return new WP_Error('not_found','Job not found.',['status'=>404]);$resource=$r['resource']==='urls'?'candidates':$r['resource'];$out=fopen('php://temp','w+');$wrote=false;$keys=[];for($offset=0;;$offset+=500){$rows=Numpo_DB::resource_rows($r['id'],$resource,500,$offset);if(!$rows)break;if(!$wrote){$keys=array_keys($rows[0]);fputcsv($out,$keys);$wrote=true;}foreach($rows as $row){$vals=[];foreach($keys as $k){$v=$row[$k]??'';$vals[]=is_scalar($v)?$v:wp_json_encode($v,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);}fputcsv($out,$vals);}if(count($rows)<500)break;}if(!$wrote)fputcsv($out,['no_results']);rewind($out);$csv=stream_get_contents($out);fclose($out);return new WP_REST_Response($csv,200,['Content-Type'=>'text/csv; charset=utf-8']);}
