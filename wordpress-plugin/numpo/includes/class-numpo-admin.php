...
function selectJob(id){if(!id)return;activeJobId=String(id);localStorage.setItem('numpo_active_job_id',activeJobId);load(activeJobId);loadHistory();} 
 ...
 async function load(id){
  ...
  try{
   ...
   root.innerHTML='<div class="np-grid"><div class="np-card np-span-12"><div class="np-section-title"><h2>Control</h2><span class="np-pill">'+textStatus+'</span></div><div class="np-actions"><button class="np-btn np-primary" id="np-start-label" disabled>🟢 شروع Crawl</button>'+(status==='running'||status==='queued'?'<button class="np-btn np-ghost" id="np-pause">⏸️ توقف موقت</button>':'')+(status==='paused'||stale?'<button class="np-btn np-primary" id="np-resume">▶️ ادامه Crawl</button>':'')+(status==='running'||status==='queued'||status==='paused'?'<button class="np-btn np-danger" id="np-cancel">⛔ لغو Job</button>':'')+'<button class="np-btn np-ghost" id="np-monitor-refresh">🔄 بروزرسانی</button></div></div><div class="np-card np-span-12"><div class="np-section np-view">Live Monitor</div><h2>مانیتورینگ زنده · Job '+esc(j.job_id)+'</h2><div class="np-progress"><i style="width:'+pct+'%"></i></div><div class="np-muted">صفحات: '+esc(j.processed_pages||0)+' / '+esc(j.max_pages||0)+' · URLها: '+esc(j.processed_urls||0)+' / '+esc(j.max_urls||0)+' · '+pct+'%</div><div class="np-grid" style="margin-top:14px">'+cards.map(x=>'<div class="np-stat np-span-3"><span>'+esc(x[0])+'</span><b>'+esc(x[1]||0)+'</b></div>').join('')+'</div></div><div class="np-card np-span-6"><div class="np-section np-view">Worker</div><h3>وضعیت Worker</h3><div class="np-kv"><strong>Status</strong><span>'+textStatus+'</span><strong>در حال بررسی</strong><span class="np-url">'+esc(m.current_url||'—')+'</span><strong>صف باقی‌مانده</strong><span>'+esc(m.queued||0)+'</span><strong>Retry</strong><span>'+esc(m.retryable||0)+'</span><strong>Deep crawled</strong><span>'+esc(m.completed||0)+'</span><strong>Last heartbeat</strong><span>'+esc(fmtDate(m.last_heartbeat))+'</span><strong>اجرای بعدی Worker</strong><span>'+esc(m.next_worker_at?fmtDate(new Date(Number(m.next_worker_at)*1000).toISOString()):"در صف زمان‌بندی")+'</span></div></div><div class="np-card np-span-6"><div class="np-section np-view">Activity</div><h3>فعالیت زنده</h3><div class="np-log">'+logs+'</div></div><div class="np-card np-span-12"><div class="np-section np-view">Completed</div><h2>✅ Crawl completed</h2><div class="np-grid"><div class="np-stat np-span-3"><span>سایت</span><b>'+esc(m.domains||0)+'</b></div><div class="np-stat np-span-3"><span>صفحه</span><b>'+esc(j.processed_pages||0)+'</b></div><div class="np-stat np-span-3"><span>URL / داده</span><b>'+esc(m.candidates||j.processed_urls||0)+'</b></div></div><div class="np-actions"><a class="np-btn np-ghost" href="'+exportUrl('urls')+'">دانلود URLها</a><a class="np-btn np-ghost" href="'+exportUrl('domains')+'">دانلود سایت‌ها</a><a class="np-btn np-primary" href="'+exportUrl('zip')+'">دانلود نتایج کامل</a></div></div>':'')+'</div>';
   const resultsCard='<div class="np-card np-span-12"><div class="np-section np-view">Results</div><h2>نتایج Crawl · Job '+esc(j.job_id)+'</h2><div class="np-tabs">'+['candidates','domains','pages','technologies','contacts','business','social','classifications','probes','errors'].map((x,i)=>'<button class="np-tab '+(i===0?'active':'')+'" data-tab="'+x+'">'+x+'</button>').join('')+'</div><div id="np-tab-body"></div></div>';
   const exportCard='<div class="np-card np-span-12"><div class="np-section np-view">Export Center</div><h2>دانلود نتایج Job '+esc(j.job_id)+'</h2><div class="np-actions"><a class="np-btn np-ghost" href="'+exportUrl('urls')+'">🔗 URLهای پیدا شده · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('txt')+'">🔗 URLها · TXT</a><a class="np-btn np-ghost" href="'+exportUrl('domains')+'">🌐 سایت‌های بررسی‌شده · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('pages')+'">📄 صفحات پردازش‌شده · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('technologies')+'">🧩 تکنولوژی‌ها · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('contacts')+'">📞 شماره‌ها و ایمیل‌ها · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('business')+'">🏢 اطلاعات کسب‌وکار · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('social')+'">📱 Social Profiles · CSV</a><a class="np-btn np-ghost" href="'+exportUrl('errors')+'">❌ خطاهای Crawl · CSV</a><a class="np-btn np-primary" href="'+exportUrl('zip')+'">📦 کل نتایج Job · ZIP</a></div></div>';
