...
class Numpo_Admin {
 ...
 <div class="wrap"><h1>Numpo</h1><p>WordPress control plane</p><h2>Create Discovery Job</h2>
 <form id="numpo-form"><p><label>Project ID<br><input name="project_id" class="regular-text" required></label></p>
 ...
 <p><label>Domains / URLs, one per line<br><textarea name="seeds" rows="8" class="large-text"></textarea></label></p>
 <p><button class="button button-primary">Start discovery</button></p></form><pre id="numpo-result"></pre></div>
 <script>
 document.getElementById('numpo-form').addEventListener('submit',async e=>{e.preventDefault();let f=new FormData(e.target);let seeds=f.get('seeds').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);let r=await fetch(ajaxurl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'numpo_admin_create',nonce:'<?php echo esc_js(wp_create_nonce('numpo_admin')); ?>',project_id:f.get('project_id'),mode:f.get('mode'),seeds:JSON.stringify(seeds)})});document.getElementById('numpo-result').textContent=await r.text();});
 </script><?php }
