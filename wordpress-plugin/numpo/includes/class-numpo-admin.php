...
<div class="wrap" id="numpo-app">
 ...
 <nav class="np-nav">
  <a href="#np-start">شروع Crawl</a><a href="#np-history">Job History</a><a href="#np-monitor">مانیتورینگ</a><a href="#np-results">نتایج</a><a href="#np-export">خروجی‌ها</a><a href="#np-settings">تنظیمات</a><a href="#np-runtime">وضعیت سیستم</a>
 </nav>
...
(function(){
 ...
 async function loadHistory(){
  ...
  try{const response=await fetch(restBase+'jobs?limit=50',{headers:{'X-WP-Nonce':restNonce}});const data=await parseResponse(response);const items=data.items||[];
   ...
   historyRoot.innerHTML='<div class="np-history-list">'+items.map(j=>'<button type="button" class="np-history-row '+(String(j.job_id)===String(activeJobId)?'active':'')+'" data-job-id="'+esc(j.job_id)+'"><span class="np-history-main"><b>Job '+esc(j.job_id)+'</b><small>'+esc(j.project_id)+' · '+esc(j.mode)+' · '+esc(fmtDate(j.created_at))+'</small></span><span class="np-history-metrics"><b>'+esc(j.processed_pages||0)+' pages</b><b>'+esc(j.processed_urls||0)+' URLs</b></span><span class="np-history-status">'+jobStatusLabel(j.status)+'</span></button>').join('')+'</div>';
 ...
 async function load(id){
  ...
  try{
   ...
   const textStatus={running:'🟢 در حال اجرا',queued:'🟠 در صف',paused:'⏸️ متوقف موقت',completed:'✅ تکمیل',failed:'🔴 خطا',cancelled:'⛔ لغو'}[status]||status;
   ...
   const cards=[['Domains',m.domains],['Candidates / URL',m.candidates],['Pages',m.pages||j.processed_pages],['Technologies',m.technologies],['Contacts',m.contacts],['Business',m.business],['Social',m.social],['Classifications',m.classifications],['Probes',m.probes],['Errors',m.errors],['Queue',m.queued],['Retry',m.retryable]];
   const logs=(m.current_url?'<div class="np-log-row"><span>اکنون</span><b class="np-good">●</b><div class="np-url">'+esc(m.current_url)+'</div></div>':'')+(m.last_candidate?'<div class="np-log-row"><span>آخرین Candidate</span><b class="np-good">+</b><div class="np-url">'+esc(m.last_candidate)+'</div></div>':'')+(m.last_error?'<div class="np-log-row"><span>خطای اخیر</span><b class="np-bad">!</b><div>'+esc(m.last_error)+'</div></div>':'')||'<div class="np-log-row"><span>—</span><b>·</b><div>هنوز Activity ثبت نشده است.</div></div>';
   root.innerHTML='<div class="np-grid"><div class="np-card np-span-12"><div class="np-section-title"><h2>Control</h2><span class="np-pill">'+textStatus+'</span></div><div class="np-actions"><button class="np-btn np-primary" id="np-start-label" disabled>🟢 شروع Crawl</button>'+(status==='running'||status==='queued'?'<button class="np-btn np-ghost" id="np-pause">⏸️ توقف موقت</button>':'')+(status==='paused'?'<button class="np-btn np-primary" id="np-resume">▶️ ادامه</button>':'')+(status==='running'||status==='queued'||status==='paused'?'<button class="np-btn np-danger" id="np-cancel">⛔ لغو Job</button>':'')+'<button class="np-btn np-ghost" id="np-monitor-refresh">🔄 بروزرسانی</button></div></div><div class="np-card np-span-12"><div class="np-section">Live Monitor</div><h2>مانیتورینگ زنده · Job '+esc(j.job_id)+'</h2><div class="np-progress"><i style="width:'+pct+'%"></i></div><div class="np-muted">صفحات: '+esc(j.processed_pages||0)+' / '+esc(j.max_pages||0)+' · URLها: '+esc(j.processed_urls||0)+' / '+esc(j.max_urls||0)+' · '+pct+'%</div><div class="np-grid" style="margin-top:14px">'+cards.map(x=>'<div class="np-stat np-span-3"><span>'+esc(x[0])+'</span><b>'+esc(x[1]||0)+'</b></div>').join('')+'</div></div><div class="np-card np-span-6"><div class="np-section">Worker</div><h3>وضعیت Worker</h3><div class="np-kv"><strong>Status</strong><span>'+textStatus+'</span><strong>در حال بررسی</strong><span class="np-url">'+esc(m.current_url||'—')+'</span><strong>صف باقی‌مانده</strong><span>'+esc(m.queued||0)+'</span><strong>Retry</strong><span>'+esc(m.retryable||0)+'</span><strong>Completed</strong><span>'+esc(m.completed||0)+'</span></div></div><div class="np-card np-span-6"><div class="np-section">Activity</div><h3>فعالیت زنده</h3><div class="np-log">'+logs+'</div></div><div class="np-card np-span-12"><div class="np-section">Completed</div><h2>✅ Crawl completed</h2><div class="np-grid"><div class="np-stat np-span-3"><span>سایت</span><b>'+esc(m.domains||0)+'</b></div><div class="np-stat np-span-3"><span>صفحه</span><b>'+esc(j.processed_pages||0)+'</b></div><div class="np-stat np-span-3"><span>URL / داده</span><b>'+esc(m.candidates||j.processed_urls||0)+'</b></div></div><div class="np-actions"><a class="np-btn np-ghost" href="'+exportUrl('urls')+'">دانلود URLها</a><a class="np-btn np-ghost" href="'+exportUrl('domains')+'">دانلود سایت‌ها</a><a class="np-btn np-primary" href="'+exportUrl('zip')+'">دانلود نتایج کامل</a></div></div>':'')+'</div>';
   const resultsCard="<div class=\"np-card np-span-12\"><div class=\"np-section\">Results</div><h2>نتایج Crawl</h2><div class=\"np-tabs\">'+['candidates','domains','pages','technologies','contacts','business','social','classifications','probes','errors'].map((x,i)=>'<button class=\"np-tab '+(i===0?'active':'')+'\" data-tab=\"'+x+'\">'+x+'</button>').join('')+'</div><div id=\"np-tab-body\"></div></div>"; const exportCard="<div class=\"np-card np-span-12\"><div class=\"np-section\">Export Center</div><h2>دانلود نتایج Job</h2><div class=\"np-actions\"><a class=\"np-btn np-ghost\" href=\"'+exportUrl('urls')+'\">🔗 URLهای پیدا شده · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('txt')+'\">🔗 URLها · TXT</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('domains')+'\">🌐 سایت‌های بررسی‌شده · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('pages')+'\">📄 صفحات پردازش‌شده · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('technologies')+'\">🧩 تکنولوژی‌ها · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('contacts')+'\">📞 شماره‌ها و ایمیل‌ها · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('business')+'\">🏢 اطلاعات کسب‌وکار · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('social')+'\">📱 Social Profiles · CSV</a><a class=\"np-btn np-ghost\" href=\"'+exportUrl('errors')+'\">❌ خطاهای Crawl · CSV</a><a class=\"np-btn np-primary\" href=\"'+exportUrl('zip')+'\">📦 کل نتایج Job · ZIP</a></div></div>'+(status==='completed'?'"; if(resultsRoot)resultsRoot.innerHTML=resultsCard; if(exportRoot)exportRoot.innerHTML=exportCard; if(resultsRoot){const tabs=resultsRoot.querySelectorAll('.np-tab');tabs.forEach(b=>b.onclick=()=>{tabs.forEach(x=>x.classList.remove('active'));b.classList.add('active');loadTab(b.dataset.tab,j)});} loadTab('candidates',j);
