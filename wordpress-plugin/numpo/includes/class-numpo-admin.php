...
class Numpo_Admin {
 ...
 function render(j){
  ...
  const running=status==='running',queued=status==='queued',done=status==='completed',fail=status==='failed',cancel=status==='cancelled';
  const statusClass=running?'running':done?'done':fail?'fail':queued?'queue':'';
  const statusText=running?'🟢 در حال اجرا':queued?'🟠 در صف شروع':done?'✅ تکمیل شد':fail?'🔴 خطا':cancel?'⛔ لغو شد':status;
  let logs='';
 ...
 async function poll(){if(!active)return;try{const j=await api('/jobs/'+encodeURIComponent(active));render(j);document.getElementById('np-runtime').textContent='Live · '+new Date().toLocaleTimeString();if(j.status==='running'||j.status==='queued'){clearTimeout(timer);timer=setTimeout(poll,5000)}else{clearTimeout(timer);timer=null}}catch(e){root.innerHTML='<div class="np-card np-bad">'+esc(e.message)+'</div>';clearTimeout(timer);timer=null}}
