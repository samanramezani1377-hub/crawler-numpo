...
(function(){
 ...
 async function load(id){
  activeJobId=id;
  try{
   const j=await api('/jobs/'+encodeURIComponent(id)),m=j.metrics||{};
   const total=Math.max(1,Number(j.max_urls||1)),processed=Number(m.processed_urls||0),pct=Math.min(100,Math.round(processed/total*100));
   const statusText={queued:'🟠 در صف',running:'🟢 در حال اجرا',paused:'⏸️ متوقف موقت',completed:'✅ تکمیل شد',failed:'🔴 خطا',cancelled:'⛔ لغو شد'}[j.status]||j.status;
   const exportBase='<?php echo esc_url(admin_url('admin-ajax.php')); ?>?action=numpo_admin_export&nonce=<?php echo esc_js(wp_create_nonce('numpo_admin')); ?>&job_id='+encodeURIComponent(id)+'&resource=';
   const resources=['urls','domains','pages','technologies','contacts','business','social','classifications','probes','errors'];
   root.innerHTML='<div class="numpo-card" style="padding:18px"><div style="display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><h2 style="margin:0">مانیتورینگ Job</h2><span class="numpo-pill">'+esc(statusText)+'</span></div><p><strong>'+esc(j.job_id||id)+'</strong> · '+esc(j.mode||'')+'</p><div style="height:12px;background:#eee;border-radius:99px;overflow:hidden"><div style="height:100%;width:'+pct+'%;background:#6d5dfc"></div></div><p>'+esc(processed)+' / '+esc(j.max_urls)+' URL · '+esc(pct)+'%</p><div class="numpo-runtime"><div class="numpo-runtime-item"><strong>Candidates</strong><div>'+esc(m.candidates||0)+'</div></div><div class="numpo-runtime-item"><strong>Queue</strong><div>'+esc(m.queued||0)+'</div></div><div class="numpo-runtime-item"><strong>Processing</strong><div>'+esc(m.processing||0)+'</div></div><div class="numpo-runtime-item"><strong>Domains</strong><div>'+esc(m.domains||0)+'</div></div><div class="numpo-runtime-item"><strong>Pages</strong><div>'+esc(m.pages||0)+'</div></div><div class="numpo-runtime-item"><strong>Technologies</strong><div>'+esc(m.technologies||0)+'</div></div><div class="numpo-runtime-item"><strong>Contacts</strong><div>'+esc(m.contacts||0)+'</div></div><div class="numpo-runtime-item"><strong>Facts</strong><div>'+esc(m.facts||0)+'</div></div><div class="numpo-runtime-item"><strong>Errors</strong><div>'+esc(m.errors||0)+'</div></div></div><p><strong>در حال بررسی:</strong> '+esc(m.current_url||'—')+'</p><p><strong>آخرین URL:</strong> '+esc(m.last_candidate||'—')+'</p><p><strong>آخرین خطا:</strong> '+esc(m.last_error||'—')+'</p><div class="numpo-actions">'+(j.status==='running'||j.status==='queued'?'<button class="button" id="numpo-pause">⏸ توقف موقت</button>':'')+(j.status==='paused'?'<button class="button button-primary" id="numpo-resume">▶ ادامه</button>':'')+(j.status==='running'||j.status==='queued'||j.status==='paused'?'<button class="button" id="numpo-cancel">■ لغو Job</button>':'')+'</div><h3>دانلود نتایج</h3><div class="numpo-actions">'+resources.map(x=>'<a class="button" href="'+exportBase+x+'">'+x.toUpperCase()+' CSV</a>').join('')+'</div></div>';
   const cancel=document.getElementById('numpo-cancel');if(cancel)cancel.onclick=async()=>{if(!confirm('Job لغو شود؟'))return;try{await api('/jobs/'+encodeURIComponent(id)+'/cancel',{method:'POST'});await load(id);}catch(e){showError(e);}};
   const pause=document.getElementById('numpo-pause');if(pause)pause.onclick=async()=>{try{await api('/jobs/'+encodeURIComponent(id)+'/pause',{method:'POST'});await load(id);}catch(e){showError(e);}};
   const resume=document.getElementById('numpo-resume');if(resume)resume.onclick=async()=>{try{await api('/jobs/'+encodeURIComponent(id)+'/resume',{method:'POST'});await load(id);}catch(e){showError(e);}};
  }catch(e){showError(e);}
 }
