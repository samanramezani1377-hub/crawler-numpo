...
(function(){
 ...
 async function load(id){
  ...
  try{
   ...
   root.innerHTML='<div class="np-grid"><div class="np-card np-span-12"><div class="np-statusbar"><div><div class="np-section">Live Crawl Dashboard</div><h2>Job '+esc(j.job_id)+'</h2><div class="np-status '+esc(status)+'">'+textStatus+' · <span id="np-last-poll">live</span></div></div><div class="np-actions">'+(status==='running'||status==='queued'?'<button class="np-btn np-ghost" id="np-pause">⏸ توقف</button>':'')+(status==='paused'?'<button class="np-btn np-primary" id="np-resume">▶ ادامه</button>':'')+(status==='running'||status==='queued'||status==='paused'?'<button class="np-btn np-danger" id="np-cancel">■ لغو</button>':'')+'</div></div><div class="np-progress"><i style="width:'+pct+'%"></i></div><div class="np-muted">پیشرفت صفحات: '+esc(j.processed_pages||0)+' از '+esc(j.max_pages||0)+' · URLها: '+esc(j.processed_urls||0)+' از '+esc(j.max_urls||0)+' · '+pct+'%</div></div><div class="np-card np-span-12"><div class="np-grid">'+cards.map(x=>'<div class="np-stat np-span-3"><span>'+esc(x[0])+'</span><b>'+esc(x[1]||0)+'</b></div>').join('')+'</div></div><div class="np-card np-span-6"><div class="np-section">Worker</div><h3>وضعیت Worker</h3><div class="np-kv"><strong>Status</strong><span>'+textStatus+'</span><strong>در حال بررسی</strong><span class="np-url">'+esc(m.current_url||'—')+'</span><strong>صف باقی‌مانده</strong><span>'+esc(m.queued||0)+'</span><strong>Retry</strong><span>'+esc(m.retryable||0)+'</span><strong>Completed</strong><span>'+esc(m.completed||0)+'</span></div></div><div class="np-card np-span-6"><div class="np-section">Live Activity</div><h3>Activity</h3><div class="np-log">'+logs+'</div></div><div class="np-card np-span-12"><div class="np-section">Export Center</div><h3>دانلود نتایج این Job</h3><div class="np-actions">'+['urls','domains','pages','technologies','contacts','business','social','classifications','probes','errors'].map(x=>'<a class="np-btn np-ghost" href="'+exportUrl(x)+'">'+x.toUpperCase()+' CSV</a>').join('')+'</div></div><div class="np-card np-span-12"><div class="np-tabs">'+['candidates','domains','pages','technologies','contacts','business','social','classifications','probes','errors'].map((x,i)=>'<button class="np-tab '+(i===0?'active':'')+'" data-tab="'+x+'">'+x+'</button>').join('')+'</div><div id="np-tab-body"></div></div></div>';
