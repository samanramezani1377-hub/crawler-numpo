...
class Numpo_Admin {
 ...
 <script>
document.getElementById('numpo-form').addEventListener('submit',async e=>{e.preventDefault();let f=new FormData(e.target);let seeds=f.get('seeds').split(/\r?\n/).map(x=>x.trim()).filter(Boolean);let sources={search_provider:f.has('source_search'),sitemap:f.has('source_sitemap'),robots:f.has('source_robots'),link_discovery:f.has('source_links'),subdomain_from_crawl:f.has('source_subdomains')};let r=await fetch(ajaxurl,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({action:'numpo_admin_create',nonce:'<?php echo esc_js(wp_create_nonce('numpo_admin')); ?>',project_id:f.get('project_id'),mode:f.get('mode'),seeds:JSON.stringify(seeds),sources:JSON.stringify(sources)})});document.getElementById('numpo-result').textContent=await r.text();});
 </script><?php }
