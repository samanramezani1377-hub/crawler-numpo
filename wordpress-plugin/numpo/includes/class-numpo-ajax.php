...
add_action('wp_ajax_numpo_admin_proxy',function(){
 ...
 $method=((substr($path,-7)==='/cancel')||$path==='/jobs')?'POST':'GET';$req=new WP_REST_Request($method,'/numpo/v1'.$path);$parts=explode('/',trim($path,'/'));$r=null;
 ...
 elseif(count($parts)>=2&&$parts[0]==='jobs'){$req->set_param('id',$parts[1]);if(count($parts)>=3&&$parts[2]==='cancel')$r=Numpo_API::cancel($req);elseif(count($parts)>=3&&$parts[2]==='errors')$r=Numpo_API::errors($req);
...
add_action('wp_ajax_numpo_admin_export',function(){
 if(!current_user_can('manage_options')||!check_ajax_referer('numpo_admin','nonce',false))wp_die('Numpo export access denied.',403);$id=sanitize_text_field(wp_unslash($_GET['job_id']??''));if($id==='')wp_die('Numpo export failed: missing Job ID.',400);$r=Numpo_API::export_csv($id);if(is_wp_error($r))wp_die(esc_html($r->get_error_message()),$r->get_error_data()['status']??500);$response=$r->get_data();nocache_headers();header('Content-Type:text/csv; charset=utf-8');header('Content-Disposition: attachment; filename="numpo-candidates.csv"');echo $response;exit;
});