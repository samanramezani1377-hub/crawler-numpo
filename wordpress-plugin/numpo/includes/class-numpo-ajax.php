...
add_action('wp_ajax_numpo_admin_export',function(){
 if(!current_user_can('manage_options')||!check_ajax_referer('numpo_admin','nonce',false))wp_die('Numpo export access denied.',403);$id=sanitize_text_field(wp_unslash($_GET['job_id']??''));$resource=sanitize_key(wp_unslash($_GET['resource']??'urls'));if($id==='')wp_die('Numpo export failed: missing Job ID.',400);$req=new WP_REST_Request('GET','/numpo/v1/jobs/'.$id.'/export/'.$resource);$req->set_param('id',$id);$req->set_param('resource',$resource);$r=Numpo_API::export_resource($req);if(is_wp_error($r))wp_die(esc_html($r->get_error_message()),$r->get_error_data()['status']??500);$response=$r->get_data();nocache_headers();$type=($resource==='zip')?'application/zip':(($resource==='txt')?'text/plain; charset=utf-8':'text/csv; charset=utf-8');$ext=($resource==='zip')?'zip':(($resource==='txt')?'txt':'csv');header('Content-Type:'.$type);header('Content-Disposition: attachment; filename="numpo-job-'.$id.'-'.$resource.'.'.$ext.'"');echo $response;exit;
});